...
import type { ExamAnswer, ExamQuestion } from "./Exam";
...
const SUBJECT_COLORS: Record<string, { bg: string; text: string; border: string; bar: string }> = {
  ...
  Physics: { bg: "bg-zinc-50 dark:bg-zinc-950/30", text: "text-zinc-900 dark:text-zinc-400", border: "border-zinc-200 dark:border-zinc-700", bar: "bg-zinc-700" },
  ...
  "General Knowledge": { bg: "bg-zinc-50 dark:bg-zinc-900/30", text: "text-zinc-800 dark:text-zinc-300", border: "border-zinc-200 dark:border-zinc-700", bar: "bg-zinc-500" },
};

function getSubjectColor(subj: string) {
  return SUBJECT_COLORS[subj] ?? { bg: "bg-gray-50 dark:bg-zinc-950", text: "text-gray-700 dark:text-zinc-300", border: "border-gray-200 dark:border-zinc-700", bar: "bg-gray-400" };
}
...
function isQuestionCorrect(question: ExamQuestion, answer: ExamAnswer) {
...
function hasAnswerKey(question: ExamQuestion) {
  return isIntegerQuestion(question) ? Boolean(question.numericAnswer) : question.correctAnswer >= 0;
}
...
export default function Results({ candidateName, rollNumber, examTitle, questions, answers, timeTaken, onRetake, isDark, onToggleDark }: ResultsProps) {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    else if (isQuestionCorrect(questions[i], answers[i])) correct++;
    else wrong++;
  }
  ...
  questions.forEach((q, i) => {
    ...
    else if (isQuestionCorrect(q, answers[i])) s.correct++;
    else s.wrong++;
  });
  ...
  const getGrade = () => {
    ...
    if (percentage >= 70) return { grade: "B+", label: "Very Good", color: "text-zinc-700 dark:text-zinc-300" };
    if (percentage >= 60) return { grade: "B", label: "Good", color: "text-zinc-600 dark:text-zinc-300" };
  ...
  return (
    <div className="min-h-screen bg-zinc-100 dark:bg-black p-4">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="bg-black text-white rounded-xl p-5 mb-4 shadow-lg">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <div>
              <div className="text-zinc-400 text-xs uppercase tracking-wide">Exam Completed</div>
              <h1 className="text-xl font-bold">{examTitle}</h1>
              <div className="text-zinc-400 text-sm">{candidateName} | Roll: {rollNumber}</div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button onClick={onRetake} className="bg-white text-zinc-900 font-semibold px-5 py-2 rounded-lg text-sm hover:bg-zinc-50 transition-colors shadow">
                New Test
              </button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div className="bg-white dark:bg-zinc-950 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 p-6 flex flex-col items-center justify-center">
            ...
            <div className="text-center -mt-24 mb-12">
              <div className="text-3xl font-black text-gray-800 dark:text-zinc-100">{correct}</div>
              <div className="text-gray-400 dark:text-zinc-500 text-xs">out of {totalMarks}</div>
          ...
          <div className="sm:col-span-2 grid grid-cols-2 gap-3">
            {[
              ...
              { value: unattempted, label: "Unattempted", color: "text-gray-400 dark:text-zinc-500", bar: "bg-gray-300" },
            ].map((item) => (
              <div key={item.label} className="bg-white dark:bg-zinc-950 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 p-4 flex flex-col items-center justify-center">
                <div className={`text-3xl font-black ${item.color}`}>{item.value}</div>
                <div className="text-gray-500 dark:text-zinc-400 text-xs mt-1">{item.label}</div>
                <div className="w-full bg-gray-100 dark:bg-zinc-900 rounded-full h-1.5 mt-2">
                  <div className={`${item.bar} h-1.5 rounded-full`} style={{ width: `${(item.value / totalMarks) * 100}%` }} />
                </div>
              </div>
            ))}
            <div className="bg-white dark:bg-zinc-950 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 p-4 flex flex-col items-center justify-center">
              <div className="text-3xl font-black text-zinc-700 dark:text-zinc-300">{formatTime(timeTaken)}</div>
              <div className="text-gray-500 dark:text-zinc-400 text-xs mt-1">Time Taken</div>
            </div>
          </div>
        </div>

        {/* Subject-wise breakdown */}
        <div className="bg-white dark:bg-zinc-950 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 mb-4 overflow-hidden">
          <div className="p-4 border-b border-gray-100 dark:border-zinc-800">
            <h2 className="font-bold text-gray-800 dark:text-zinc-100">Subject-wise Performance</h2>
          </div>
                ...
                <div key={subj} className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      <span className="text-gray-400 dark:text-zinc-500">{s.unattempted} skipped</span>
                      <span className="font-bold text-gray-700 dark:text-zinc-200">{pct}%</span>
                    </div>
                  </div>
                  <div className="w-full bg-gray-100 dark:bg-zinc-900 rounded-full h-2">
        ...
        <div className="bg-white dark:bg-zinc-950 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 overflow-hidden">
          <div className="p-4 border-b border-gray-100 dark:border-zinc-800">
            <h2 className="font-bold text-gray-800 dark:text-zinc-100">Review Answers & Solutions</h2>
            <p className="text-gray-500 dark:text-zinc-400 text-xs mt-0.5">Click a subject to see all questions with correct answers</p>
          </div>
            ...
            <div className="border-t border-gray-100 dark:border-zinc-800">
              {questions
                ...
                .map(({ q, globalIdx }) => {
                  ...
                  const integerQuestion = isIntegerQuestion(q);
                  const isCorrect = isQuestionCorrect(q, userAnswer);
                  const isSkipped = userAnswer === null;
                  const hasKey = hasAnswerKey(q);
                  return (
                    <div key={q.id} className={`p-4 border-b border-gray-50 dark:border-zinc-800 ${isCorrect ? "bg-green-50/40 dark:bg-green-900/10" : isSkipped ? "bg-gray-50/60 dark:bg-black/30" : "bg-red-50/40 dark:bg-red-900/10"}`}>
                          ...
                          {integerQuestion && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                              <div className={`text-xs px-2.5 py-1.5 rounded-lg border ${isCorrect ? "bg-green-100 dark:bg-green-900/50 border-green-400 dark:border-green-700 text-green-800 dark:text-green-300 font-semibold" : !isSkipped ? "bg-red-100 dark:bg-red-900/50 border-red-400 dark:border-red-700 text-red-800 dark:text-red-300" : "bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-300"}`}>
                                Your answer: {typeof userAnswer === "string" && userAnswer ? userAnswer : "Not attempted"}
                              </div>
                              <div className="text-xs px-2.5 py-1.5 rounded-lg border bg-green-100 dark:bg-green-900/50 border-green-400 dark:border-green-700 text-green-800 dark:text-green-300 font-semibold">
                                Correct answer: {q.numericAnswer || "Not found"} {hasKey && <span className="ml-1 text-green-600 dark:text-green-400">✓</span>}
                              </div>
                            </div>
                          )}
        ...
        <div className="mt-4 text-center">
          <button onClick={onRetake} className="bg-zinc-950 hover:bg-black text-white font-semibold px-8 py-3 rounded-xl text-sm transition-colors shadow-sm">
            Start New Test
          </button>
        </div>
      </div>
